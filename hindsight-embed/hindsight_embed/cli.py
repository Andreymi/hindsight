...
def _do_configure_from_env():
    ...
    base_url = os.environ.get("HINDSIGHT_EMBED_LLM_BASE_URL", default_base_url or "")
    bank_id = os.environ.get("HINDSIGHT_EMBED_BANK_ID", "default")

    print()
    print("\033[1m\033[36m  Hindsight Embed - Non-interactive Configuration\033[0m")
    print()
    print(f"  \033[2mProvider:\033[0m {actual_provider}")
    if base_url:
    ...
    with open(CONFIG_FILE, "w") as f:
        ...
        f.write(f"HINDSIGHT_EMBED_LLM_PROVIDER={actual_provider}\n")
        f.write(f"HINDSIGHT_EMBED_LLM_MODEL={model}\n")
        if base_url:
            f.write(f"HINDSIGHT_EMBED_LLM_BASE_URL={base_url}\n")
...
def _do_configure_interactive():
    ...
    providers = [
        ("OpenAI", "openai"),
        ("OpenAI-compatible (Ollama Cloud, etc.)", "openai-compatible"),
    ...
    if env_key:
        ...
        # OpenAI-compatible always needs API key
        api_key = _prompt_password("Enter your API key")
        if not api_key:
            print("\n\033[31m✗\033[0m API key is required.", file=sys.stderr)
            return 1
