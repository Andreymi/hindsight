...
# Provider choices for interactive configure: (provider_id, default_model, env_key_name, default_base_url)
PROVIDER_DEFAULTS = {
    "openai": ("openai", get_default_model_for_provider("openai"), "OPENAI_API_KEY", None),
    "openai-compatible": ("openai", "gpt-4o-mini", None, "https://ollama.com/v1"),
    "groq": ("groq", get_default_model_for_provider("groq"), "GROQ_API_KEY", None),
    "gemini": ("gemini", get_default_model_for_provider("gemini"), "GEMINI_API_KEY", None),
    "ollama": ("ollama", get_default_model_for_provider("ollama"), None, "http://localhost:11434/v1"),
    "vertexai": ("vertexai", get_default_model_for_provider("vertexai"), None, None),
}
...
def _do_configure_from_env():
    ...
    _, default_model, env_key, default_base_url = PROVIDER_DEFAULTS[provider]

    # Check for API key (required for providers that need one)
    # vertexai uses GCP service account credentials; openai-compatible may not need a key
    if not api_key and provider not in ("ollama", "vertexai", "openai-compatible"):
    ...
    base_url = os.environ.get("HINDSIGHT_API_LLM_BASE_URL", default_base_url)
    bank_id = os.environ.get("HINDSIGHT_EMBED_BANK_ID", "default")
    ...
    if base_url:
        print(f"  \033[2mBase URL:\033[0m {base_url}")
    print(f"  \033[2mBank ID:\033[0m {bank_id}")

    # Save configuration
    CONFIG_DIR.mkdir(parents=True, exist_ok=True)

    # For openai-compatible, save actual_provider (openai) not the alias
    actual_provider, _, _, _ = PROVIDER_DEFAULTS[provider]

    with open(CONFIG_FILE, "w") as f:
        f.write("# Hindsight Embed Configuration\n")
        f.write("# Generated by hindsight-embed configure (non-interactive)\n\n")
        f.write(f"HINDSIGHT_API_LLM_PROVIDER={actual_provider}\n")
        ...
        if base_url:
            f.write(f"HINDSIGHT_API_LLM_BASE_URL={base_url}\n")
...
def _do_configure_interactive(profile_name: str | None = None, port: int | None = None):
    ...
    actual_provider, default_model, env_key, default_base_url = PROVIDER_DEFAULTS[provider]
    print()
    ...
    # Base URL (for openai-compatible, ollama, or custom endpoints)
    base_url = ""
    if provider in ("openai-compatible", "ollama"):
        base_url = _prompt_text("Base URL", default=default_base_url or "")
        if base_url is None:
            return 1
        print()
    ...
    # Prepare config dict (use actual_provider for the config, not the alias)
    config_dict = {
        "HINDSIGHT_API_LLM_PROVIDER": actual_provider,
    ...
    if base_url:
        config_dict["HINDSIGHT_API_LLM_BASE_URL"] = base_url
...
def main():
    ...
    if remaining_args:
        ...
        # Check for LLM API key (not required for vertexai/ollama/openai-compatible)
        llm_provider = config.get("llm_provider", "openai")
        providers_without_api_key = ("ollama", "vertexai", "openai-compatible")
