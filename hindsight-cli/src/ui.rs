...
///
/// When stdout is not a TTY (piped, captured by a tool harness, redirected),
/// the spinner becomes a no-op: no animation thread is spawned and no bytes
/// are written. Otherwise `\r`-based frame redraws would accumulate as
/// literal "Recalling memories..." lines in the consumer's buffer.
...
impl GradientSpinner {
    ...
    pub fn finish(&mut self) {
        ...
        if self.interactive {
            // Clear the line only on a real terminal — otherwise the clear
            // sequence itself would show up in the captured output.
